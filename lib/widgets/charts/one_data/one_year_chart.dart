...
class YearChart extends StatelessWidget {
  const YearChart({super.key, required this.yearlyData});

  final List<double> yearlyData;

  @override
  Widget build(BuildContext context) {
    double maxData = yearlyData.reduce((a, b) => a > b ? a : b);
    double adjustedMaxY = (maxData / 20000).ceil() * 20000;

    return LayoutBuilder(
      builder: (context, constraints) {
        double barWidth = constraints.maxWidth * 0.05; // Adjust bar width
        ...
        return Padding(
          ...
          child: BarChart(
            BarChartData(
              ...
              barGroups: List.generate(yearlyData.length, (i) {
                bool isCurrentYear = i == yearlyData.length - 1; // Current year is the last one in the list
